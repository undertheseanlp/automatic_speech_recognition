...
START=$(date +%s);
steps/train_sat.sh --cmd "$train_cmd" 2500 20000 \
  data/train data/lang exp/tri3a_ali exp/tri4a || exit 1;
END=$(date +%s);
TRI4A_TRAINING_TIME=$((END - START))
...
END=$(date +%s);
TRI4A_ALIGNMENT_TIME=$((END - START))
...
START=$(date +%s);
steps/train_sat.sh --cmd "$train_cmd" 3500 100000 \
  data/train data/lang exp/tri4a_ali exp/tri5a || exit 1;
END=$(date +%s);
TRI5A_TRAINING_TIME=$((END - START))
...
END=$(date +%s);
TRI5A_ALIGNMENT_TIME=$((END - START))
...
steps/train_ubm.sh --cmd "$train_cmd" 500 data/train data/lang exp/tri5a_ali exp/ubm5b2 || exit 1;
...
steps/train_sgmm2.sh --cmd "$train_cmd" 5000 12000 \
  data/train data/lang exp/tri5a_ali exp/ubm5b2/final.ubm exp/sgmm2_5b2 || exit 1;
END=$(date +%s);
...
echo "Tri4a" >> $log_file
echo $TRI4A_TRAINING_TIME | awk '{print int($1/60)":"int($1%60)}' >> $log_file
echo $TRI4A_DECODING_TIME | awk '{print int($1/60)":"int($1%60)}' >> $log_file
echo $TRI4A_ALIGNMENT_TIME | awk '{print int($1/60)":"int($1%60)}' >> $log_file

echo "Tri5a" >> $log_file
echo $TRI5A_TRAINING_TIME | awk '{print int($1/60)":"int($1%60)}' >> $log_file
echo $TRI5A_DECODING_TIME | awk '{print int($1/60)":"int($1%60)}' >> $log_file
echo $TRI5A_ALIGNMENT_TIME | awk '{print int($1/60)":"int($1%60)}' >> $log_file
